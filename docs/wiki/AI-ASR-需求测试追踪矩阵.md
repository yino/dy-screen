...
本文逐项追踪 OpenSpec 变更 `add-user-triggered-ai-asr` 的三份 delta spec。状态定义：
...
只有六项报告均存在、不可覆盖、哈希一致且各自 `allPassed=true`（质量报告按定义通过）后，
才能把 OpenSpec 进度从 `77/83` 更新为 `83/83` 并归档变更。
