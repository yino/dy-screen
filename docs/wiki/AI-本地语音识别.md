...
本页聚焦本地 ASR 子系统：它只把一批视频转换为可定位、可复用、只读的文本结果，不会把字幕
写进视频，也不会在识别过程中调用 LLM、自动裁剪或渲染成品。

当前 AI 工作区已经提供独立的 DeepSeek 精彩分析。该步骤只在 ASR 完成且用户再次明确授权后
运行，使用规范化文本和稳定句段 ID，不属于本地 ASR 引擎，也不会作为识别失败的自动回退。
完整说明见 [AI 精彩分析与候选](AI-精彩分析与候选.md)。
...
React 工作区位于 `ui/src/AiWorkspace.tsx`。前端只持有脱敏 DTO、一次性文件授权 ID、稳定项目/输入/句段 ID，不接收源路径或模型 stderr。精彩运行、批次、候选和选择通过独立的类型化命令读取，不改变 ASR 产物。
...
ASR 结果不提供专业图形时间轴。左侧是有序视频和状态，主区域是当前播放器与时间戳文本：
...
- 转写本身只允许浏览、复制、TXT/JSON 导出，不允许编辑、拆分、合并、拖动或裁剪；精彩候选
  只引用既有稳定句段并保存用户选择，不改写转写。
...
精彩工作流读取已经原子发布的规范化句段、稳定句段 ID、源视频内时间和输入顺序。它不会读取
临时 WAV、Whisper 原始 JSON、模型 stderr 或预览缓存。预览缺失时仍可完成 LLM 分析和候选
选择，只是播放器跳转与范围播放不可用。

ASR 与精彩分别维护缓存指纹：ASR 由源指纹和识别配置决定；精彩由转写、输入顺序、标签、
Skills、模型、Prompt 版本和参数决定。修改精彩标签不会重新执行 ASR，修改源视频或识别配置也
不会错误复用旧转写。两者的详细用户流程从 [Wiki 首页](首页.md) 进入。
