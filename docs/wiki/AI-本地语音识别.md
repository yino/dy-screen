...
当前 AI 工作区已经提供独立的 DeepSeek 高光分析。该步骤只在 ASR 完成且用户再次明确授权后
运行，使用规范化文本和稳定句段 ID，不属于本地 ASR 引擎，也不会作为识别失败的自动回退。
完整说明见 [AI 高光分析与候选](AI-高光分析与候选.md)。
...
React 工作区位于 `ui/src/AiWorkspace.tsx`。前端只持有脱敏 DTO、一次性文件授权 ID、稳定项目/输入/句段 ID，不接收源路径或模型 stderr。高光运行、批次、候选和选择通过独立的类型化命令读取，不改变 ASR 产物。
...
- 转写本身只允许浏览、复制、TXT/JSON 导出，不允许编辑、拆分、合并、拖动或裁剪；高光候选
...
高光工作流读取已经原子发布的规范化句段、稳定句段 ID、源视频内时间和输入顺序。它不会读取
临时 WAV、Whisper 原始 JSON、模型 stderr 或预览缓存。预览缺失时仍可完成 LLM 分析和候选
选择，只是播放器跳转与范围播放不可用。

ASR 与高光分别维护缓存指纹：ASR 由源指纹和识别配置决定；高光由转写、输入顺序、标签、
Skills、模型、Prompt 版本和参数决定。修改高光标签不会重新执行 ASR，修改源视频或识别配置也
