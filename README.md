...
当前桌面客户端版本为 `0.3.0`，交付“可靠录制 + 用户触发的本地 ASR + 精彩候选分析 + 可校对逐字字幕的单轨剪辑 + 同步转场素材”能力。精彩和转场匹配通过 Rust 中的受限 Agent 工作流调用 DeepSeek，只发送用户授权的规范化转写、时间戳、标签、分析目标和本地素材语义；用户可编排原始片段与带声桥接素材并导出新的 H.264/AAC MP4，多轨和自动切片仍属于后续独立能力。
...
- WebView 能自行进入公开直播页时静默恢复监听；确实需要交互时显示一次带“立即验证”和“稍后处理”的系统操作弹窗，点击主操作直接打开并聚焦验证窗口；
...
- 为主播持久保存可调整的录制优先级；直播主播超过额度时，按优先级选择录制目标，未入选主播保持“直播中、等待录制资源”并继续监听；
- 录制额度只接受 AppStart 或心跳响应的可选 `max_screen_limit` 正整数；从未收到合法字段时默认 4，设置页只读展示当前服务端额度；
...
- 精彩分析使用版本化通用、带货、搞笑、知识和故事 Skills，未知标签只作为数据，不可改变 Agent 工具边界；
...
- 原始转写和录像保持只读、不生成 SRT/ASS，也不会覆盖原视频；已选择精彩可进入单轨剪辑页，在工程副本中校对或隐藏字幕并导出带逐字烧录字幕的新 MP4，但仍不提供波形、多轨、关键帧、用户自定义素材或字幕样式配置；
- 主播、设置、视频、ASR 和精彩结果均保存在本机；客户端只向授权运营服务上报白名单启动/功能/错误埋点，不上传媒体、ASR 文本、本地路径、Cookie 或直播页面正文。
...
│       ├── AiWorkspace.tsx      AI 项目、精彩候选、播放器与独立剪辑界面
...
10. 如果系统弹窗或监控中心显示“需要访问验证”，点击“立即验证”并在抖音窗口完成人工操作；选择“稍后处理”时主界面横幅会持续保留；
11. 在监控列表使用上下箭头调整录制优先级；直播数量超过服务端额度时，优先录制序号靠前的在线主播，离线主播不占名额；
12. 主播开播时自动录制，下播时自动结束会话。
...
FFprobe → FFmpeg → Silero VAD → AsrEngine
         ...
         稳定句段 ID + 源内/项目时间戳 + 文本
                                  ...
                                  └─→ 用户授权 → Candidate Agent → Ranking Agent → 精彩候选
```
...
ASR 完成后，打开设置页填写 DeepSeek 模型和 API Key，点击“测试连接”验证凭据；回到已完成项目，确认发送范围后点击“开始精彩分析”。分析只发送规范化文本和相对时间，不发送视频、音频、本地路径、Cookie 或签名流地址。候选结果的时间和句段 ID 由 Rust 本地校验，选择保存只写入 SQLite，不会自动触发 FFmpeg；用户可在“已选择”中点击“编辑视频”创建可恢复的单轨工程，再显式导出新的 MP4。

剪辑字幕直接复用每个来源视频已经发布的规范化 ASR 句段，不会再次识别。首次创建或打开旧工程时，应用会把相交句段投影为当前工程独立的字幕副本；后续修正、隐藏和恢复只影响该工程，不修改原始 ASR，也不会传播到其他剪辑工程。句段越过片段边界时会先裁剪，再映射到当前工程时间；重排或移除片段后自动重新计算。

剪辑工作台的视频轨下方提供不可拖动调时的字幕轨。点击字幕块可定位播放器并打开右侧“字幕”属性；字幕面板支持当前片段或全工程范围、原文和修正文案搜索、播放位置高亮以及长列表独立滚动。修改文本后在编辑框失焦或按 `Cmd/Ctrl + Enter` 保存，按 `Escape` 放弃草稿；也可以隐藏、恢复显示或恢复单条 ASR 原文。版本冲突时草稿会保留，必须重新加载最新工程后再提交。导出期间字幕和片段结构均锁定。

播放器和导出共同使用 Rust 生成的逐字显示计划：按 Unicode 字素簇处理中文、组合字符和 emoji，标点跟随相邻文字出现；句段前 85% 时长逐字显示，后 15% 保持完整字幕。长句会按输出画幅稳定分页，每页最多两行，逐字出现时不会重新换行。固定样式为底部居中的白色半粗体、黑色描边和轻微阴影，不绘制背景遮罩；当前版本不允许配置字体、颜色、位置或样式，也不接受外部字幕文件。

每个剪辑片段必须至少关联一条非空 ASR 字幕副本。缺少字幕来源时仍可预览、重排或移除片段，但界面和后端都会阻止导出，并提示先补全 ASR 或移除该片段；用户主动隐藏已有字幕属于明确编辑选择，不会单独阻止导出。原始录像、ASR 产物和预览缓存始终只读，导出只生成用户指定的新 MP4。已经导出完成的工程发生字幕修改后，旧 MP4 文件会保留，但不再代表当前工程版本，界面会恢复“导出 MP4”主操作并要求重新生成。
...
所有已开启监听的主播都会持续检查直播状态。目标集合只从当前在线主播中按 `recording_priority`、主播 ID 的稳定顺序取前 N 位；优先级或服务端额度变化时会先安全停止被移出的录制并保留完成分片，许可实际释放后才为新目标重新解析签名地址。AppStart/心跳缺少字段或请求失败不会把最近合法额度重置为 4。
...
- `streamers`：来源类型、来源 URL、`profile_sec_uid`、`web_rid`、当前 `room_id`、监听开关、状态及持久录制优先级；
...
- `ai_highlight_runs`、`ai_highlight_chunks`、`ai_highlight_candidates`：精彩运行快照、分块状态、结构化评分和用户选择；
...
未来切片功能可以读取结构化的 `StreamerPromptContext`，从 ASR 文本中优先寻找商品表达或搞笑表达相关句子。当前版本只保存和展示标签，不生成 Prompt、不调用 LLM、不执行 ASR、精彩评分、句子提取或视频切片，也不会自动根据主页或录制内容推断标签。
...
| `make test-ai` | 执行 ASR 调度、精彩 repository、凭据 fake 和 Skills 单测 |
...
- AI 工作区支持只读原始时间戳文本、DeepSeek 精彩评分、合格/优秀阈值、已选择候选的单轨剪辑工程、工程级字幕校对和逐字烧录的 H.264/AAC MP4 导出；仍不支持修改原始转写、SRT/ASS、说话人分离、真实字级时间戳、字幕拆分合并或调时、波形、多轨、外部素材、字幕样式配置或对原始录像的就地修改；
