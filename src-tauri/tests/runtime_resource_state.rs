...
use dy_screen::runtime_resources::{
    RuntimeArchive, RuntimeComponent, RuntimeFile, RuntimeInstaller, RuntimeManifest,
    RuntimeManifestSignature, RuntimePlatformManifest, sha256_file,
};
...
fn fixture_manifest(root: &Path, bundle_version: &str, contents: &[u8]) -> RuntimeManifest {
    ...
    let (size_bytes, sha256) = sha256_file(&tool).unwrap();
    RuntimeManifest {
        schema_version: 2,
